...
# Copy Device Specific GO-Super Gamepad.cfg udev autoconfig
RA_AUTO="/run/muos/storage/retroarch/autoconfig/udev/GO-Super Gamepad.cfg"
if [ -f "$RA_AUTO" ]; then
	rm -f "$RA_AUTO"
	cp "$DEVICE_CONTROL_DIR/GO-Super Gamepad.cfg" "$RA_AUTO"
fi
