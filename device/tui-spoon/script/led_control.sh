...
if [ $# -lt 8 ] || [ $# -gt 11 ]; then
	echo "Usage: $0 <led_mode 1–7> <brightness (max 60)> <left_r> <left_g> <left_b> <right_r> <right_g> <right_b> [middle_r] [middle_g] [middle_b]"
	exit 1
fi
...
# Clamp brightness to max 60... no idea why 60
...
echo "1000" >"$LED_SYSFS/effect_duration_l"
echo "1000" >"$LED_SYSFS/effect_duration_r"
echo "1000" >"$LED_SYSFS/effect_duration_m"

echo "-1" >"$LED_SYSFS/effect_cycles_l"
echo "-1" >"$LED_SYSFS/effect_cycles_r"
echo "-1" >"$LED_SYSFS/effect_cycles_m"

echo "$EFFECT_TYPE" >"$LED_SYSFS/effect_l"
echo "$EFFECT_TYPE" >"$LED_SYSFS/effect_r"
echo "$EFFECT_TYPE" >"$LED_SYSFS/effect_m"
