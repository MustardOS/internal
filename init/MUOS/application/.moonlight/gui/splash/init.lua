...
function splashlib.new(init)
  ...
  self.muos = {
    sprite = love.graphics.newImage(current_folder .. "/muos.png"),
    scale = 0,
    rot   = 0
  }
  ...
  timer.script(function(wait)
    ...
    timer.tween(0.2, colors.pink, {0, 0, 0}, "linear")
	timer.tween(0.2, colors.blue, {0, 0, 0}, "linear")
    timer.tween(0.2, self.heart, {scale = 0}, "in-quad", nil, 1, 1)
    timer.tween(0.4, self.stripes, {rot = 0}, "out-back", function()
      timer.tween(0.6, self.muos, {scale = 1}, "out-elastic", nil, 1, 0.4)
    end)
...
function splashlib:draw()
  ...
  self.canvas:renderTo(function()
    ...
    love.graphics.draw(
     ...
     -- Draw the heart sprite
    love.graphics.setColor(1, 1, 1, self.muos.scale)
    love.graphics.draw(
      self.muos.sprite,
      0, 0,
      self.muos.rot,
      self.muos.scale/2 * (height / 600),
      self.muos.scale/2 * (height / 600),
     146, 150
    )
