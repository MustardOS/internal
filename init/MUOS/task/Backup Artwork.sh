...
DEST_FILE="$DEST_DIR/Artwork-$(date +%Y-%m-%d).zip"
MUOS_CAT_DIR="$DC_STO_ROM_MOUNT/MUOS/info/catalogue"

cd /
echo "Archiving Artwork" >/tmp/muxlog_info
zip -ru9 "$DEST_FILE" "$MUOS_CAT_DIR" >"$TMP_FILE" 2>&1 &
