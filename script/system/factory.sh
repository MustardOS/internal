...
LOG_INFO "$0" 0 0 "FACTORY RESET" "Setting date time to default"
date 010100002025
hwclock -w
...
LOG_INFO "$0" 0 0 "FACTORY RESET" "Starting Hotkey Daemon"
/opt/muos/script/mux/hotkey.sh &
/usr/bin/mpv /opt/muos/share/media/factory.mp3 &

if [ "$(GET_VAR "device" "board/network")" -eq 1 ]; then
	LOG_INFO "$0" 0 0 "FACTORY RESET" "Generating SSH Host Keys"
	/opt/openssh/bin/ssh-keygen -A &
fi

LOG_INFO "$0" 0 0 "FACTORY RESET" "Setting ARMHF Requirements"
if [ ! -f "/lib/ld-linux-armhf.so.3" ]; then
	LOG_INFO "$0" 0 0 "BOOTING" "Configuring Dynamic Linker Run Time Bindings"
	ln -s /lib32/ld-linux-armhf.so.3 /lib/ld-linux-armhf.so.3
fi
ldconfig -v >"/opt/muos/ldconfig.log"

LOG_INFO "$0" 0 0 "FACTORY RESET" "Initialising Factory Reset Script"
