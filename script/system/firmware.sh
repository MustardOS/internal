...
if [ "$FIRMWARE_DONE" -eq 0 ]; then
	...
	dd if=/opt/muos/"$DEVICE"/firmware/boot.bin of="$FW_BOOT_OUT" seek="$FW_BOOT_SEEK" conv=notrunc
	dd if=/opt/muos/"$DEVICE"/firmware/package.bin of="$FW_PACK_OUT" seek="$FW_PACK_SEEK" conv=notrunc
