...
udevadm settle --timeout=30 || LOGGER "$0" 0 "BOOTING" "Udevadm Settle Failure"

if [ "$(GET_VAR "global" "boot/factory_reset")" -eq 0 ]; then
	LOGGER "$0" 0 "BOOTING" "Loading Storage Mounts"
	/opt/muos/script/mount/start.sh &

	LOGGER "$0" 0 "BOOTING" "Removing any update scripts"
	rm -rf /opt/update.sh
...
LOGGER "$0" 0 "BOOTING" "Restoring Default Sound System"
cp -f "/opt/muos/device/current/control/asound.conf" "/etc/asound.conf"

if [ -s "$ALSA_CONFIG" ]; then
	LOGGER "$0" 0 "BOOTING" "ALSA Config Check Passed"
else
	LOGGER "$0" 0 "BOOTING" "ALSA Config Check Failed: Restoring"
	cp -f "/opt/muos/config/alsa.conf" "$ALSA_CONFIG"
fi

LOGGER "$0" 0 "BOOTING" "Restoring Audio State"
cp -f "/opt/muos/device/current/control/asound.state" "/var/lib/alsa/asound.state"
alsactl -U restore

LOGGER "$0" 0 "BOOTING" "Starting Pipewire"
...
if [ "$(GET_VAR "global" "boot/factory_reset")" -eq 1 ]; then
	...
	LOGGER "$0" 0 "FACTORY RESET" "Setting date time to default"
	date 101100002024
	hwclock -w
	...
	LOGGER "$0" 0 "FACTORY RESET" "Starting Hotkey Daemon"
	/opt/muos/script/mux/hotkey.sh &
	/usr/bin/mpg123 -q /opt/muos/factory.mp3 &

	if [ "$(GET_VAR "device" "board/network")" -eq 1 ]; then
		LOGGER "$0" 0 "FACTORY RESET" "Generating SSH Host Keys"
		/opt/openssh/bin/ssh-keygen -A &
	fi

	LOGGER "$0" 0 "FACTORY RESET" "Setting ARMHF Requirements"
	if [ ! -f "/lib/ld-linux-armhf.so.3" ]; then
		LOGGER "$0" 0 "BOOTING" "Configuring Dynamic Linker Run Time Bindings"
		ln -s /lib32/ld-linux-armhf.so.3 /lib/ld-linux-armhf.so.3
	fi
	ldconfig -v >"/opt/muos/ldconfig.log"

	LOGGER "$0" 0 "FACTORY RESET" "Initialising Factory Reset Script"
	/opt/muos/script/system/reset.sh

	LOGGER "$0" 0 "FACTORY RESET" "Switching off Factory Reset mode"
...
LOGGER "$0" 0 "BOOTING" "Starting Low Power Indicator"
/opt/muos/script/system/lowpower.sh &

LOGGER "$0" 0 "BOOTING" "Precaching RetroArch System"
ionice -c idle /opt/muos/bin/vmtouch -tfb /opt/muos/preload.txt &

LOGGER "$0" 0 "BOOTING" "Running Device Specifics"
/opt/muos/device/current/script/start.sh

LOGGER "$0" 0 "BOOTING" "Starting Hotkey Daemon"
/opt/muos/script/mux/hotkey.sh &
...
LOGGER "$0" 0 "BOOTING" "Waiting for Storage Mounts"
while [ ! -f /run/muos/storage/mounted ]; do
	sleep 0.25
done

LOGGER "$0" 0 "BOOTING" "Detecting Charge Mode"
/opt/muos/device/current/script/charge.sh

LOGGER "$0" 0 "BOOTING" "Setting Device Controls"
/opt/muos/device/current/script/control.sh &

# Set the device specific SDL Controller Map
LOGGER "$0" 0 "BOOTING" "Setting up SDL Controller Map"
/opt/muos/script/mux/sdl_map.sh &

LOGGER "$0" 0 "BOOTING" "Checking for passcode lock"
HAS_UNLOCK=0
if [ "$(GET_VAR "global" "settings/advanced/lock")" -eq 1 ]; then
	while [ "$HAS_UNLOCK" != 1 ]; do
		LOGGER "$0" 0 "BOOTING" "Enabling passcode lock"
		nice --20 /opt/muos/extra/muxpass -t boot
		HAS_UNLOCK="$?"
	done
fi

LOGGER "$0" 0 "BOOTING" "Starting USB Function"
/opt/muos/script/system/usb.sh &

LOGGER "$0" 0 "BOOTING" "Bringing up localhost network"
ifconfig lo up &

LOGGER "$0" 0 "BOOTING" "Checking for network capability"
if [ "$(GET_VAR "device" "board/network")" -eq 1 ] && [ "$(GET_VAR "global" "network/enabled")" -eq 1 ]; then
	LOGGER "$0" 0 "BOOTING" "Starting Network Services"
	/opt/muos/script/system/network.sh &
fi

LOGGER "$0" 0 "BOOTING" "Running catalogue generator"
...
LOGGER "$0" 0 "BOOTING" "Correcting Home permissions"
chown -R root:root /root &
chmod -R 755 /root &

LOGGER "$0" 0 "BOOTING" "Correcting SSH permissions"
chown -R root:root /opt &
chmod -R 755 /opt &

echo 2 >/proc/sys/abi/cp15_barrier &

LOGGER "$0" 0 "BOOTING" "Backing up global configuration"
/opt/muos/script/system/config_backup.sh &

LOGGER "$0" 0 "BOOTING" "Starting muX frontend"
