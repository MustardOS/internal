...
TASK_BEGIN "toggle_grid_mode_main_menu" "Toggle Grid Mode Main Menu"


INI_FILE="$MUOS_STORE_DIR/theme/override/muxlaunch.ini"
GRID_SECTION="[grid]"
...
if grep -qFx "$GRID_SECTION" "$INI_FILE"; then
    if grep -qFx "$COLUMN_SETTING" "$INI_FILE" && grep -qFx "$ROW_SETTING" "$INI_FILE"; then
		TASK_STATUS "Enabling Grid Mode for Main Menu"
		RESULT="on"
        sed -i "/$COLUMN_SETTING/d" "$INI_FILE"
        sed -i "/$ROW_SETTING/d" "$INI_FILE"
    else
		TASK_STATUS "Disabling Grid Mode for Main Menu"
		RESULT="off"
...
if [ "$RESULT" = "on" ]; then
	TASK_COMPLETE "Grid Mode enabled for Main Menu"
else
	TASK_COMPLETE "Grid Mode disabled for Main Menu"
fi
